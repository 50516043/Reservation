...
public class MySQL {
 ...
 // データベースの指定
    ...
    public ResultSet getConfirm(String reservation_userid){
    	          //SELECT * FROM 
    ...
    public ResultSet getfacilityExplanation(String facility_name){
    	        //SELECT * FROM 
    ...
    public int CancelReservation(String rdate, String st, String et, String reservation_userid, String facility){
    	 int rs_int = 0;
    	             //DELETE FROM uriage WHERE 
    	 String sql = "DELETE FROM reservation WHERE date ='" + rdate + "'AND start_time = '" + st + "'AND end_time = '" 
    	 + et + "'AND user_id = '" + reservation_userid + "'AND facility_name = '" + facility +"';";
    	 try {
			rs_int = stmt.executeUpdate(sql);
		} catch (Exception e) {
			// TODO: handle exception
