...
public class Reservation_control {
	...
	////// 新規予約の登録
		...
		public String getConfirm(Reservation_view frame){
			...
			if(flagLogin){
				try {
					...
					ResultSet rs = mysql.getConfirm(reservation_userid);
					boolean exist = false;
					while (rs.next()) {
						String start = rs.getString("start_time");
		...
		public String getfacilityExplanation(Reservation_view frame){
			...
			String res = "";
			 ...
			 try {
				ResultSet rs;
				rs = mysql.getfacilityExplanation(frame.choiceFacility.getSelectedItem());
				while (rs.next()) {
